...
//     - get list of topic lists on planning board
...
const getCurrentPeriod = (enddate) => {
  var now = new Date();
  var dayDiff = 7 - now.getDay();
  var endOfWeek = enddate ? new Date(enddate) : addDays(now, dayDiff);
  var startOfWeek = addDays(endOfWeek, -6);
...
const cardsTimeInList = (cards, idList, period) =>
  cards.map((card) => {
    ...
    return {
      ...
      created: created,
      timeInList: dhm(period.enddate - enteredList),
      age: dhm(period.enddate - created),
    };
  });

const cardsNewThisPeriod = (cards, idList, periodend) =>
  cardsTimeInList(cards, idList, periodend).sort((a, b) => a.enteredList - b.enteredList)[0];

const inProgressMetricDashboard = (periodend) => (data) => {
  var inProgress = data[0];
  var done = data[1];
  var topics = (data[2] || []).map((x) => x.name).filter((x) => x !== 'Ready');
  var initialIdeas = data[3];
  var researchIdeas = data[4];
  var ideasForConsideration = data[5];

  var allCards = inProgress.cards.concat(done.cards) || [];

  var period = inspect(getCurrentPeriod(periodend));
  ...
  return {
    ...
    cardsByType: cardsByLabel(allCards, ['bootstrap', 'experiment', 'vision']),
    cardsByTopic: cardsByLabel(allCards, topics),
